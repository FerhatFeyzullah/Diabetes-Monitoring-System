...
export const LoginPost = createAsyncThunk('login', async (data, { rejectWithValue }) => {
    try {
        const response = await axios.post("Auths/Login", data);
        return response.data;
    } catch (error) {
        return rejectWithValue(error.response?.data || { message: "Bağlantı hatası veya bilinmeyen bir hata oluştu" });
    }
});
